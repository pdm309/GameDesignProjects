...
public class Player2Controller : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        wintext = GameObject.FindGameObjectWithTag("Text").GetComponent<Text>();
        wintext.text = "";
        Debug.Log("text: " + wintext);
        bulletArray = GameObject.FindGameObjectsWithTag("Bullet 2");
    }
    ...
    void Update()
    {
        ...
        frame++;
        frameOf60 = frame % 60;
        if (Input.GetKeyDown(KeyCode.UpArrow) && alive) //jumping
        {
            if (ableToJump)
            {
        ...
        if (Input.GetKey(KeyCode.RightArrow) && alive) //moving right
        {
        ...
        if (Input.GetKey(KeyCode.LeftArrow) && alive) //moving left
        {
        ...
        if (!Input.GetKey(KeyCode.LeftArrow) && !Input.GetKey(KeyCode.RightArrow) && alive && ableToJump) //standing still
        {
            spriteR.sprite = sprites[23];
        }


        if (Input.GetKey(KeyCode.DownArrow) && bullets > 0 && shootingCooldown == 0) //warp
        {
            bullets--;
    ...
    void OnCollisionStay2D(Collision2D obj)
    {
        ...
        if (obj.gameObject.tag == "ProcGround" || obj.gameObject.tag == "Player 1")
        {
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            if (player.shooting[0] || player.shooting[1] || player.shooting[2])
            {
